...
__all__ = ['load_whole_click_data', 'BASE_DIR']
...
#     now_phase = 2
    ...
    for c in range(now_phase + 1):
        ...
        click_train1 = pd.read_csv(train_path + '/underexpose_train_click-{}.csv'.format(c), header=None,  names=['user_id', 'item_id', 'time'], converters={'time': str})
        click_test1 = pd.read_csv(test_path + '/underexpose_test_click-{}/underexpose_test_click-{}.csv'.format(c, c), header=None,  names=['user_id', 'item_id', 'time'], converters={'time': str})
        test_qtime1 = pd.read_csv(test_path + '/underexpose_test_click-{}/underexpose_test_qtime-{}.csv'.format(c, c), header=None,  names=['user_id','time'], converters={'time': str})
...
#         click_test_val = click_test_val.append(click_test1_val, ignore_index=True)
    ...
    whole_click = whole_click.sort_values('time').drop_duplicates(['user_id', 'item_id'], keep='last').reset_index(drop=True)
...
#     del whole_click['query_time']
    ...
    logger.info(f'whole_click_train: {whole_click_train.shape}, whole_click_val: {whole_click_val.shape}, test_qtime: {test_qtime.shape}')
    return whole_click_train, whole_click_val, test_qtime

# Cell
BASE_DIR = '../../data/kdd_debiasing/data_origin'